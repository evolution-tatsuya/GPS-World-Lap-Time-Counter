...
## 追補 v2.2 — 販売化に向けた要件確定（2026-08-03）

> 富士スピードウェイ実走検証（2026-08-01〜03）を経て、販売を見据えたゴール像を確定。詳細は `docs/PRODUCT_VISION.md` を正とする。本追補はその要点を要件定義に取り込むもの。

### A. ロールと権限
- ロールは既存の **DRIVER / ORGANIZER / ADMIN** を用いる。
- **コース/GPS登録は ORGANIZER も可、ただし公開にはADMIN承認**（申請中→検証済み）。DRIVERはGPS/コース登録UIを持たない。
- 参加者の閲覧範囲: **自分は全タイム、他人はランキングのベストラップのみ**（現状維持）。

### B. コース資産・計測
- 階層検索: **国 → スポーツ → コース種別 → コース名**（既存Course列を活用）。
- **片道モード対応**: Course に「計測タイプ(LAP/ONE_WAY)」とゴールライン(GA/GB)列を追加（単体HTML版で実装済みの概念をDBへ）。
- アウトラップ=LAP1、セッション管理は単体HTML版の実働仕様を踏襲・移植。

### C. 記録・運営機能
- **記録の受け皿を フルアプリDB(Neon) に集約**（GAS/スプレッド方式は単体HTML版のサンプルのみ残す）。ラップは `POST /api/laps`。堅牢送信（未送信キュー＋再送）を踏襲。
- **運営モニター**: ①リアルタイム通過モニター（トランスポンダー風）＋②集計・ランキング表 の両方。
- **エクスポート**: CSV / ZIP / 印刷（Googleアカウント不要）。
- **統括ページ(ADMIN)**: アカウント（運営/参加者のID・パスワード・詳細）、コース、イベント、国・スポーツ・名前など**登録済み全データのCRUD**。

### D. 多言語（i18n）
- まず **日本語・英語・中国語・韓国語** の4言語。**react-i18next 等でキー参照＋言語ファイル**方式。他言語は言語ファイル追加で拡張。
- **言語選択はログイン前画面**（ID/パスワード入力前）。選択はブラウザ保存で維持。
- タイム/LAP/BEST等の共通用語・数値は**英語固定**、UI説明・ボタンを翻訳。運営/参加者向けは「英語＋カッコ書き選択言語」併記も可。

### E. 配布・課金
- **配布: PWA化**（manifest＋Service Worker、オフライン対応）。将来ネイティブ化（iOS画面ロック根絶）。
- **課金: サブスク主軸**。参加者 300〜500円/月、運営者 月額3,000円 or 都度2,000円（選択制）。決済連携(Stripe等)は将来Phase。
- DBコスト試算: 運営100・利用者1000で月$0〜$19（テレメトリーはスマホ内保存でDB非依存）。

### F. プロモ枠（オーナー専用・課金免除）
- オーナー専用の**課金免除運営アカウント**。1イベント最大100台まで無料。
- 参加は**イベントコード/QR**（100人共有可、個人ID/パス不要＝記録されて困る情報なし）。イベント終了/無効化でアクセス終了。
- 用途: **参加者へのPR＋運営者への営業デモ**（動くデモで契約獲得）。
- 実装: プロモ枠配下のイベントは**課金判定をバイパス**する分岐を1本入れる。

### 既存実装の現状（2026-08-03 起動検証で確認）
...
### 追補 v2.3 — 2026-08-04 実装完了分（詳細はSCOPE_PROGRESS.md / PRODUCT_VISION.md）
- 統括ページ(/admin, ADMIN限定): 全イベント/全コース横断閲覧、イベント別CSV、ユーザー管理(プロモON/OFF)、コース承認/却下UI。
- プロモ枠: User.isPromo/Event.isPromo。プロモ作成イベントは自動プロモ+最大100台(将来の課金判定バイパス点)。
- 参加制御: 開催期間外は参加不可(無料体験を当日限定)、参加受付は開始24時間前から(前日準備可)。開催時間は端末ローカル時刻。
- コース承認フロー: Course.approvalStatus。統括作成は即APPROVED、運営はPENDING、承認済みのみイベント作成可。
- 個人利用モード: Lap.eventIdをnullable化。ログインユーザーがイベントなしで承認済みコースを個人計測→DB保存(userIdあり/eventIdなし)→自分のベスト/履歴。POST/GET /api/laps/personal。
- ランキング画面に参加中イベント名・コード表示(設定完了の可視化)。
