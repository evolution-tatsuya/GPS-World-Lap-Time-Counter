...
- [ ] 統括ページ・運営モニター画面（既存APIで作れる範囲＝一覧/ランキング/CSVから。ユーザー管理/承認/ライブSSEはバックエンド引き継ぎ）

### 次の作業（担当エージェント別・要件はPRODUCT_VISION.md）
- **フロントエンドエンジニア**: 統括ページ・運営モニターの画面（既存API範囲）
- **バックエンドエンジニア**: プロモ枠（課金免除）、統括ページのAPI（ユーザー管理/承認）、運営ライブモニターのSSE
