...
### フルアプリ Phase 1-A（計測フロー一本化）完了（2026-08-03）
- [x] A-0 現状フロー検証（参加→計測→DB→ランキングの背骨が動作）
- [x] A-1 Lapスキーマ拡張（session/ゼッケン/クラス/タイヤ/メモ、アウトラップms=0対応、migration）
- [x] A-2 useGPSを実働仕様へ（アウトラップ=LAP1・セッション・片道2ライン、実測/シミュ共通化）
- [x] A-3 Course片道対応（measureType/ゴールライン列、event-login/session/eventsに反映、formatCourseLines共通化）
- [x] A-4 堅牢送信（localStorage未送信キュー＋自動/手動再送＋UI、eventId分離）

### フロントエンド基盤 完了（2026-08-03）
- [x] MUI v9移行（型エラー22→0、本番ビルド完走＝デプロイ可能）
- [x] i18n基盤（react-i18next、日英中韓、言語選択UI、ログイン前選択・localStorage保存）
- [x] PWA基盤（vite-plugin-pwa、manifest/SW、オフライン・ホーム追加・地図タイルキャッシュ）

### フロントエンド機能実装 進行中（2026-08-03）
- [x] 片道コース登録UI（CircuitCreateに計測モード＋ゴールライン入力、E2E検証済）
- [x] 全ページi18n化完了（参加者/入口/運営 全10画面。4言語225キー完全一致・翻訳漏れなし）
- [x] 統括ページ(/admin, ADMIN限定): 全イベント/全コース横断閲覧＋イベント別CSV出力

### バックエンド機能実装 完了（2026-08-04）
- [x] プロモ枠（課金免除）: User.isPromo/Event.isPromo、プロモ作成イベントは自動プロモ＋100台制限、ADMIN管理API。将来の課金判定バイパス点。
- [x] 統括ページ ユーザー管理: プロモ枠ON/OFFを画面から操作
- [x] 開催期間チェック: 開催時間外はイベントコードで参加不可（無料体験を当日/その回に限定）
- [x] 24時間前ルール: 参加受付は開始24時間前から（前日準備可）。isJoinWindowOpen。計測/位置共有はisWithinEventWindowで開催時間内に限定
- [x] 前日準備ガイド: EventCreateに「前日作成→当日参加」の運用説明を4言語で追加
- [x] コース承認フロー: Course.approvalStatus。統括(ADMIN)作成は即APPROVED(どんどん作れる)、運営はPENDING。承認済みのみイベント作成可。統括ページに承認/却下UI。既存コースはdefault APPROVED
- [x] ランキング画面に参加中イベント名・コード表示（設定完了の可視化）。event-login/sessionにeventCode追加

### 個人利用モード 完了（2026-08-04）
- [x] P-1 Lap.eventIdをnullable＋courseId追加（個人ラップ対応）migration。既存ラップ影響なし
- [x] P-2/P-3 個人計測API: POST/GET /api/laps/personal（ログイン者のみ、承認済みコース検証、eventIdなし保存、自分のbest/履歴）
- [x] P-4 フロント: PersonalMeasurement画面（コース選択→useGPS計測→個人保存→ベスト/履歴）、/personalルート、Dashboard導線、4言語
- [x] これで2つの利用モード（イベント利用＋個人利用）が同居。プロダクトの核が完成

### 残タスク・次フェーズ（要件はPRODUCT_VISION.md）
- **運営ライブモニター（SSE）**: ラップのリアルタイム配信（大）
- **サブスク課金・決済連携（Stripe）**: 大・お金が絡むため要相談・別フェーズ。個人計測/プロモ枠に課金判定バイパス点(コメント)を用意済み
